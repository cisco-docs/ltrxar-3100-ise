class Rule:
    ...
    def match(cls, data):
        results = []
        default_allowed_protocols = ["Default Device Admin"]
        allowed_protocols = []
        ...
        for policy_set in data.get("ise", {}).get("device_administration", {}).get("policy_sets", []):
            if policy_set.get("service_name") not in all_allowed_protocols:
                results.append(
                    f"service_name - {policy_set.get('service_name')} under policy_set - {policy_set.get('name')} is not defined in device administration -> policy_elements -> allowed_protocols data model"
                )
