*** Settings ***
Documentation   Verify Network Access Authentiction Exception Rules
Suite Setup     Login ISE
Resource        ../../ise_common.resource
Default Tags    config   ise   network_access   policy_sets   authorization_exception_rules
...
Verify Network Access Authorization Exception Rule {{ rule.name }} in {{ policy_set.name }}
    ${policy_id}=   Get Value From Json   ${r.json()}   $.response[?(@.name=='{{ policy_set.name }}')]['id']
    ${r}=   GET On Session   ISE_Session   /api/v1/policy/network-access/policy-set/${policy_id}[0]/exception
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.state   {{ rule.state | default(defaults.ise.network_access.policy_sets.authorization_exception_rules.state) | default('enabled')  }}
...
{% if condition.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.condition.isNegate   {{ condition.is_negate | default(defaults.ise.network_access.policy_sets.authorization_exception_rules.condition.is_negate) | default(false)  }}
