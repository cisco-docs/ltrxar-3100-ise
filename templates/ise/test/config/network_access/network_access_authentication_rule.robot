*** Settings ***
Documentation   Verify Network Access Authentiction Rules
Suite Setup     Login ISE
Resource        ../../ise_common.resource
Default Tags    config   ise   network_access   policy_sets   authentication_rules
...
Verify Network Access Authentication Rule {{ rule.name }} in {{ policy_set.name }}
    ${policy_id}=   Get Value From Json   ${r.json()}   $.response[?(@.name=='{{ policy_set.name }}')]['id']
    ${r}=   GET On Session   ISE_Session  /api/v1/policy/network-access/policy-set/${policy_id}[0]/authentication
    ${rule}=   Set Variable   $.response[?(@.rule.name=='{{ rule.name }}')]
    Should Be Equal Value Json String   ${r.json()}   ${rule}.identitySourceName   {{ rule.identity_source_name | default(defaults.ise.identity_management.internal_users.password_id_store) | default('Internal Users')  }}
    Should Be Equal Value Json String   ${r.json()}   ${rule}.ifAuthFail   {{ rule.if_auth_fail | default(defaults.ise.network_access.policy_sets.authentication_rules.if_auth_fail) | default('REJECT')  }}
    Should Be Equal Value Json String   ${r.json()}   ${rule}.ifUserNotFound   {{ rule.if_user_not_found | default(defaults.ise.network_access.policy_sets.authentication_rules.if_user_not_found) | default('REJECT')  }}
    Should Be Equal Value Json String   ${r.json()}   ${rule}.ifProcessFail   {{ rule.if_process_fail | default(defaults.ise.network_access.policy_sets.authentication_rules.if_process_fail) | default('DROP')  }}
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.rank   {{ rule.rank | default(loop.index0)   }}
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.state   {{ rule.state | default(defaults.ise.network_access.policy_sets.authentication_rules.state) | default('enabled')  }}
...
{% if condition.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.condition.isNegate   {{ condition.is_negate | default(defaults.ise.network_access.policy_sets.authentication_rules.condition.is_negate) | default(false)  }}
