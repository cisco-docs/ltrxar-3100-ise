*** Settings ***
Documentation   Verify Network Access Time and Date Conditions
Suite Setup     Login ISE
Resource        ../../ise_common.resource
Default Tags    config   ise   network_access   policy_elements   time_date_conditions

*** Test Cases ***

Get Network Access Time and Date Conditions
    ${r}=   GET On Session   ISE_Session   /api/v1/policy/network-access/time-condition
    Log   Response Status Code: ${r.status_code}
    Set Suite Variable   ${r}

{% for condition in ise.network_access.policy_elements.time_date_conditions | default([]) %}
Verify Network Access Time and Date Condition {{ condition.name }}
