*** Settings ***
Documentation   Verify Network Access Conditions
Suite Setup     Login ISE
Resource        ../../ise_common.resource
Default Tags    config   ise   network_access   policy_elements   conditions

*** Test Cases ***

Get Network Access Conditions
    ${r}=   GET On Session   ISE_Session   /api/v1/policy/network-access/condition
    Log   Response Status Code: ${r.status_code}
    Set Suite Variable   ${r}

{% for condition in ise.network_access.policy_elements.conditions | default([]) %}
Verify Network Access Condition {{ condition.name }}
...
{% if condition.type == 'LibraryConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${cond}.isNegate   {{ condition.is_negate | default(defaults.ise.network_access.policy_elements.conditions.is_negate) | default(false) }}
