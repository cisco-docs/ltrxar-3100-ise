...
Default Tags    config   ise   device_administration   policy_sets   authorization_rules
...
Verify Device Admin Authorization Rule {{ rule.name }} in {{ policy_set.name }}
    ${policy_id}=   Get Value From Json   ${r.json()}   $.response[?(@.name=='{{ policy_set.name }}')]['id']
    ${r}=   GET On Session   ISE_Session   /api/v1/policy/device-admin/policy-set/${policy_id}[0]/authorization
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.state   {{ rule.state | default(defaults.ise.device_administration.policy_sets.authorization_rules.state) | default('enabled') }}
...
{% if condition.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.condition.isNegate   {{ condition.is_negate | default(defaults.ise.device_administration.policy_sets.authorization_rules.condition.is_negate) | default(false) }}
