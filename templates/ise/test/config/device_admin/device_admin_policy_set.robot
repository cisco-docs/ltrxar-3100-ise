*** Settings ***
Documentation   Verify Device Admin Policy Sets
Suite Setup     Login ISE
Resource        ../../ise_common.resource
Default Tags    config   ise   device_administration   policy_sets
...
Verify Device Admin Policy Set {{ policy_set.name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${policy}.state   {{ policy_set.state | default(defaults.ise.device_administration.policy_sets.state) | default('enabled')  }}
    Should Be Equal Value Json String   ${r.json()}   ${policy}.isProxy   {{ policy_set.is_proxy | default(defaults.ise.device_administration.policy_sets.is_proxy) | default(false)  }}
...
{% if condition.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${policy}.condition.isNegate   {{ condition.is_negate | default(defaults.ise.device_administration.policy_sets.condition.is_negate) | default(false)  }}
...
{% if child.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${cond_child}.isNegate   {{ child.is_negate | default(defaults.ise.device_administration.policy_sets.condition.conditions.is_negate) | default(false)  }}
...
{% if child_child.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${cond_child_child}.isNegate   {{ child_child.is_negate | default(defaults.ise.device_administration.policy_sets.condition.conditions.conditions.is_negate) | default(false)  }}
