*** Settings ***
Documentation   Verify Device Admin Authentiction Exception Rules
Suite Setup     Login ISE
Resource        ../../ise_common.resource
Default Tags    config   ise   device_administration   policy_sets   authorization_exception_rules
...
{% for rule in policy_set.authorization_exception_rules | default([]) %}
Verify Device Admin Authorization Exception Rule {{ rule.name }} in {{ policy_set.name }}
    ${policy_id}=   Get Value From Json   ${r.json()}   $.response[?(@.name=='{{ policy_set.name }}')]['id']
    ${r}=   GET On Session   ISE_Session   /api/v1/policy/device-admin/policy-set/${policy_id}[0]/exception
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.state   {{ rule.state | default(defaults.ise.device_administration.policy_sets.authorization_exception_rules.state) | default('enabled') }}
{% if rule.condition.type == 'ConditionAttributes' %}
    ...
    Should Be Equal Value Json String   ${r.json()}   ${rule}.rule.condition.isNegate   {{ rule.condition.is_negate | default(defaults.ise.device_administration.policy_sets.authorization_exception_rules.condition.is_negate) | default(false) }}
